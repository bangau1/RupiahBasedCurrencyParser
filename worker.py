...
def getDefaultDatabasePath():
	return "/var/www/products/RupiahBasedCurrencyParser/database/currency.json";
def getDatabasePathDaily(now=time.time()):
	...
	return "/var/www/products/RupiahBasedCurrencyParser/database/{year:0>4}/{month:0>2}/currency_{date:0>2}.json".format(year=jakartaStructTime.tm_year, month=jakartaStructTime.tm_mon, date=jakartaStructTime.tm_mday);
